...
## Setup
Set env vars (load from `.env` is supported). You can start from `.env.example`.
...
Set `OPENAI_API_KEY` or `GRAPHQL_EMBED_API_KEY` in `mcp/.env` if you plan to use `list_types`.
...
## Index the schema
```bash
python3 src/schema_indexer.py  # uses schema.graphql and data/ by default
```
Outputs a count plus schema hash; rerun after editing the schema.

## Search via CLI (no server)
```bash
python3 src/schema_indexer.py search "find order mutations"
python3 src/schema_indexer.py search "product price fields" --limit 3
```
...
`run_query` will return real data from the endpoint. `list_types` still needs an embeddings API key to build/search the embedding index.
